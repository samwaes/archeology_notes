...
export async function updateRecordAction(formData: FormData) {
  ...
  const visibility = text(formData, "visibility") as Visibility;
  const status = text(formData, "status") as RecordStatus;
  const acquisitionAt = text(formData, "acquisitionAt");

  if (!recordId) throw new Error("Record ID is required.");
  if (!RECORD_TYPES.has(recordType)) throw new Error("Invalid record type.");
  if (!VISIBILITIES.has(visibility)) throw new Error("Invalid visibility.");
  if (!STATUSES.has(status)) throw new Error("Invalid record status.");
  if (!acquisitionAt) throw new Error("Acquisition date is required.");

  await updateRecord({
    recordId,
    actorId: user.localUserId,
    siteId: text(formData, "siteId") || null,
    physicalObjectId: text(formData, "physicalObjectId") || null,
    recordType,
