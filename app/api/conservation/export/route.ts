...
export async function GET(request: Request) {
  ...
  const rows = items.map((item) => [
    item.recordId, item.recordType, item.acquisitionAt, item.title || "", item.siteName || "", item.objectName || "",
    item.category || "", item.severity || "", item.confidence || "", item.treatmentPriority || "",
    item.interventionType || "", item.interventionStatus || "", item.method || "", item.materials || "",
    item.outcome || "", item.authorName || item.authorEmail, item.visibility
  ].map(csv).join(","));

  return new Response([header.join(","), ...rows].join("\n"), {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="${String(project.slug)}-conservation.csv"`,
